...
**这是硬性规则，没有任何例外。在未获得用户明确确认之前，禁止执行任何代码编写、文件创建、代码修改等开发操作。**

#### 3.1 强制确认流程

在开始任何开发工作之前，必须严格遵循以下流程，**缺一不可**：

1. **理解需求**: 仔细阅读和理解用户的需求描述
   - **检查点**: 必须完全理解用户的需求，不能有任何模糊之处
...
3. **方案说明**: 向用户详细说明你的实现方案：
   ...
   - 预期的变更范围和影响
   - 需要修改的文件列表
   - **检查点**: 方案说明必须完整、清晰，让用户充分了解将要进行的变更

4. **等待用户确认**: **⚠️ 强制等待用户明确确认，未确认前禁止任何开发操作**
   - **禁止自行判断**：Agent 不能自行判断方案是否可行或是否获得确认
   - **禁止自动确认**：不能假设用户同意或默认开始编码
   - **禁止推测**：不能根据用户的语气、表情符号等推测用户意图
   - **必须明确确认**：需要用户明确表达"确认"、"可以"、"开始"、"同意"、"好的"等明确同意意图
   - **如果用户没有明确确认**：必须继续等待或再次询问，**绝对不能开始编码**
   - **检查点**: 在开始任何开发操作前，必须再次确认已获得用户明确同意

5. **需求变更**: 如果在开发过程中发现需求需要调整：
   - **立即停止开发**：停止所有编码工作
   - **与用户确认**：说明需要调整的内容和原因
   - **等待确认**：获得用户明确确认后，才能继续开发
   - **检查点**: 任何需求变更都必须重新走确认流程

#### 3.2 严格禁止的行为（违反将被视为严重错误）

以下行为**严格禁止**，违反将导致开发流程中断：
...
- ❌ **在用户只提供信息但未明确确认时开始开发**
- ❌ **在用户提出问题时继续开发**
- ❌ **使用任何工具进行代码编写、文件创建、代码修改等操作，除非已获得明确确认**

#### 3.3 开发操作检查清单

在执行任何开发操作（使用 `write`、`search_replace`、`delete_file` 等工具）之前，必须确认：

- [ ] 已完全理解用户需求
- [ ] 已向用户说明实现方案
- [ ] **已获得用户明确确认（必须包含明确的同意词汇）**
- [ ] 没有未解答的疑问
- [ ] 没有需求变更需要确认

**如果以上任何一项未完成，禁止执行开发操作。**

#### 3.4 用户确认的标准（严格执行）

**⚠️ 重要：只有以下情况才视为已确认，其他所有情况都视为未确认**

**视为已确认的情况（必须包含明确的同意词汇）**：

- ✅ 用户明确说"确认"、"可以"、"开始"、"同意"、"好的"、"行"、"没问题"等明确同意词汇
- ✅ 用户说"按这个方案实施"、"就这样做"、"按你说的来"等明确指示
- ✅ 用户明确表示"可行"、"没问题"、"可以开始"等肯定答复
- ✅ 用户说"开始吧"、"开始开发"、"开始实现"等明确开发指令
...
- ❌ 用户保持沉默或没有回应
- ❌ Agent 自行判断用户可能同意
- ❌ 用户说"看起来不错"、"应该可以"、"可能行"等模糊表达
- ❌ 用户说"试试看"、"看看效果"等不确定表达
- ❌ 用户只回复了表情符号或简短回复（如"嗯"、"哦"等）
- ❌ 用户提出了新的问题或要求

**确认检查流程**：

1. 检查用户回复中是否包含明确的同意词汇
2. 如果没有，视为未确认，必须继续等待或再次询问
3. 如果有，但用户同时提出了问题或修改建议，视为未确认，需要先解决问题
4. 只有在确认用户明确同意且没有其他问题后，才能开始开发
...
Agent: 我理解您想要添加功能 X。我的实现方案是...
用户: 这个方案看起来不错
Agent: [开始编写代码]  ❌ 严重错误："看起来不错"不是明确确认
```
...
Agent: 我理解您想要添加功能 X。我的实现方案是...
用户: [保持沉默或只回复"嗯"]
Agent: [开始编写代码]  ❌ 严重错误：推测用户同意，未获得明确确认
```

#### 3.7 强制检查点

在执行任何开发操作前，Agent 必须进行自我检查：

1. **我是否完全理解了用户的需求？** 如果否，必须询问
2. **我是否向用户说明了实现方案？** 如果否，必须说明
3. **用户是否明确确认了方案？** 如果否，**禁止开始开发**
4. **用户是否提出了问题或修改建议？** 如果是，必须先解决问题
5. **我是否在推测用户意图？** 如果是，**禁止开始开发**
